...
columns = pl.read_csv(f"leap-atmospheric-physics-ai-climsim/train.csv", n_rows = 1).to_pandas().columns.to_list()
...
for i in range(0, len(CFG.start_num)):
    ...
    df = pl.read_csv(f"leap-atmospheric-physics-ai-climsim/train.csv", columns = cols).to_pandas()
    if i >= 10:
        sub = pl.read_csv(f"leap-atmospheric-physics-ai-climsim/sample_submission.csv", columns=cols, n_rows=1).to_numpy()
    ...
    for n in range(10):
        df.iloc[CFG.N*n:CFG.N*(n+1), :].to_parquet(f"data/data_{i}_{n}.parquet")
    
norm_data.to_parquet(f"data/norm_data.parquet")